...
public class SessionControlService {
    ...
    public Uni<SessionAuthorizationInfoModel> createSession(String xCord, String yCord, String accountPublicKey, UUID accountId, String signature, String ip) {
        return userAccountRepository.getUserAccount(accountId, this.client).onItem().transformToUni(user -> {
            ...
            return sessionControlRepository.addSession(sessionId, key, accountId, serverPrivateKey.toString(), serverPublicKey.toString(), accountPublicKey, scrambler.toString(), authorizationKey, signature, ip, this.client).onItem().transform(sessionModel -> new SessionAuthorizationInfoModel(sessionId, accountId, serverPublicKey.toString(), user.getAccountSalt()));
        });
    }

    public Uni<SessionAuthorizationConfirmModel> confirmSession(String sessionId, String confirm, String signature) {
        return sessionControlRepository.getSession(sessionId, client).onItem().transformToUni(sessionModel -> userAccountRepository.getUserAccount(sessionModel.getSessionAccountId(), this.client).onItem().transform(userAccountModel -> {
            String serverConfirm = SRP.H(new BigInteger(SRP.H(SRP.n.toString(16)), 16).xor(new BigInteger(SRP.H(SRP.g.toString(16)), 16)).toString(16) + SRP.H(userAccountModel.getAccountName()) + userAccountModel.getAccountSalt() + sessionModel.getSessionAccountPublicKey() + sessionModel.getSessionServerPublicKey() + sessionModel.getSessionAuthorizationKey());
            ...
            if (!Equals.equals(signature, confirm)) {
                throw new RuntimeException(new SessionControlServiceException("Confirm is not true", "Invalid confirm"));
            }
