...
import ru.iedt.authorization.models.session.UpdateTokenModel;
...
public class SessionControlRepository {
    ...
    public Uni<SessionModel> addSession(
        String sessionId,
        String sessionKey,
        UUID accountId,
    ...
    public Uni<UpdateTokenModel> getUpdateToken(String token, PgPool client) {
        HashMap<String, ParameterInput> parameters = new HashMap<>();
        parameters.put("SESSION_UPDATE_TOKEN", new ParameterInput("SESSION_UPDATE_TOKEN", token));
        return databaseController
            .runningQuerySet("SESSION_CONTROL", "GET_UPDATE_TOKEN", parameters, client)
            .onItem()
            .transform(element -> element.get(0).get("main"))
            .onItem()
            .transform(RowSet::iterator)
            .onItem()
            .transform(iterator -> iterator.hasNext() ? UpdateTokenModel.from(iterator.next()) : null);
    }

    public Uni<UpdateTokenModel> addUpdateToken(String token, UUID accountId, PgPool client) {
        HashMap<String, ParameterInput> parameters = new HashMap<>();
        parameters.put("SESSION_UPDATE_TOKEN", new ParameterInput("SESSION_UPDATE_TOKEN", token));
        parameters.put("SESSION_ACCOUNT_ID", new ParameterInput("SESSION_ACCOUNT_ID", accountId.toString()));
        return databaseController
            .runningQuerySet("SESSION_CONTROL", "ADD_UPDATE_TOKEN", parameters, client)
            .onItem()
            .transform(element -> element.get(0).get("main"))
            .onItem()
            .transform(RowSet::iterator)
