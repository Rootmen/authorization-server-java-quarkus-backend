...
-- ТАБЛИЦА DNAUTHORIZATION.UPDATE_TOKENS
CREATE TABLE IF NOT EXISTS DNAUTHORIZATION.UPDATE_TOKENS
(
    SESSION_ACCOUNT_ID         UUID REFERENCES DNAUTHORIZATION.USERS_ACCOUNT (ACCOUNT_ID) NOT NULL,
    SESSION_UPDATE_TOKEN       TEXT UNIQUE                                                NOT NULL,
    SESSION_UPDATE_TOKEN_START TIMESTAMP DEFAULT CURRENT_TIMESTAMP                        NOT NULL
);
-- ДОБАВЛЕНИЕ КОММЕНТАРИЕВ К ТАБЛИЦЕ DNAUTHORIZATION.UPDATE_TOKENS И ЕЁ СТОЛБЦАМ
COMMENT ON TABLE DNAUTHORIZATION.UPDATE_TOKENS IS 'Таблица со списком долгоживущих токенов';
COMMENT ON COLUMN DNAUTHORIZATION.UPDATE_TOKENS.SESSION_ACCOUNT_ID IS 'Ид токена';
COMMENT ON COLUMN DNAUTHORIZATION.UPDATE_TOKENS.SESSION_UPDATE_TOKEN IS 'Токен обновления';
COMMENT ON COLUMN DNAUTHORIZATION.UPDATE_TOKENS.SESSION_UPDATE_TOKEN_START IS 'Дата выдача токена';
