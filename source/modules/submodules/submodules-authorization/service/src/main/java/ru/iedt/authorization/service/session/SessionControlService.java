...
public class SessionControlService {
        ...
        return sessionControlRepository
                ...
                .transformToUni(tuple -> {
                    ...
                    return sessionControlRepository
                            ...
                            .onItem()
                            .transform(jwt -> new ResultConfirm(sessionId, userAccount.getAccountId(), serverOutConfirm, jwt, refreshToken));
                });
    }
        ...
        return userAccountRepository
                ...
                .transform(userAccountModel -> {
                    if (userAccountModel == null)
                        throw new AuthorizationException("Пользователь не найден", "UserAccountModel is null");
    ...
    private Uni<Void> checkUserIsBlock(UUID accountId, String ip) {
        return userAccountRepository
                ...
                .transform(isBlock -> {
                    if (isBlock.isBlock())
                        throw new SessionControlServiceException("Пользователь заблокирован на " + isBlock.getTimeToUnlock() + " мин.", "User is blocked");
                    return null;
                })
                .replaceWithVoid();
    }
